...
class BoardApiIntegrationTests {
 ...
 @Test void recordFollowsConfiguredWorkflowAndWritesAudit() throws Exception {
  long id=create("UT-BOARD-001","企业流程自动化验收");
  mvc.perform(post("/api/records/{id}/actions",id).with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"action\":\"START\",\"remark\":\"越级操作\"}")).andExpect(status().isConflict());
  mvc.perform(post("/api/records/{id}/actions",id).with(httpBasic("admin","admin123")).contentType(MediaType.APPLICATION_JSON).content("{\"action\":\"CONVENE\",\"remark\":\"第一步完成\"}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.status").value("已召集"));
  mvc.perform(post("/api/records/{id}/actions",id).with(httpBasic("admin","admin123")).contentType(MediaType.APPLICATION_JSON).content("{\"action\":\"START\",\"remark\":\"第二步完成\"}")).andExpect(status().isOk()).andExpect(jsonPath("$.data.status").value("会议中"));
 ...
 @Test void draftRecordSupportsUpdateDeleteAndModuleValidation() throws Exception {
  String body=body("UT-BOARD-002","待修改记录");
  ...
  mvc.perform(post("/api/records").with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content(body.replace("UT-BOARD-002","UT-BOARD-BAD").replace("MEETING","UNKNOWN"))).andExpect(status().isBadRequest());
 }
 ...
 @Test void searchSlaTimelineCommentsAndCsvExportAreUsable() throws Exception {
  long id=create("UT-BOARD-OPS","运营检索与协作记录");
  mvc.perform(get("/api/records/search").param("module","MEETING").param("keyword","运营检索").param("page","0").param("size","10").with(httpBasic("operator","operator123")))
   .andExpect(status().isOk()).andExpect(jsonPath("$.data.total").value(1)).andExpect(jsonPath("$.data.items[0].id").value(id));
  mvc.perform(get("/api/records/{id}",id).with(httpBasic("operator","operator123"))).andExpect(status().isOk()).andExpect(jsonPath("$.data.recordNo").value("UT-BOARD-OPS"));
  ...
  mvc.perform(get("/api/records/export.csv").param("keyword","运营检索").with(httpBasic("operator","operator123"))).andExpect(status().isOk()).andExpect(header().string("Content-Disposition",containsString("records.csv"))).andExpect(content().string(containsString("UT-BOARD-OPS")));
 }
 ...
 @Test void administratorWorkflowActionRejectsOperator() throws Exception {
  long id=create("UT-BOARD-ROLE","管理员动作权限校验");

  mvc.perform(post("/api/records/{id}/actions",id).with(httpBasic("operator","operator123")).contentType(MediaType.APPLICATION_JSON).content("{\"action\":\"CONVENE\",\"remark\":\"无权操作\"}")).andExpect(status().isForbidden());
  mvc.perform(post("/api/records/{id}/actions",id).with(httpBasic("admin","admin123")).contentType(MediaType.APPLICATION_JSON).content("{\"action\":\"CONVENE\",\"remark\":\"授权操作\"}")).andExpect(status().isOk());
 }
  ...
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
 private String body(String no,String title){return "{\"recordNo\":\""+no+"\",\"module\":\"MEETING\",\"title\":\""+title+"\",\"businessParty\":\"上海总部\",\"owner\":\"业务专员\",\"amount\":12800,\"quantity\":3,\"dueDate\":\"2026-09-30\",\"riskLevel\":\"正常\",\"description\":\"验证主要功能、状态控制和审计\"}";}
 /**
  * 商业授权或定制开发请微信添加微信号zhuatech或zhuatech2进行咨询。
  */
